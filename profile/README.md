...
- 입력 처리 (홀센서 기반 악셀/브레이크, 가변 저항 핸들, 사용자 버튼 입력)
- 입력부 → 중앙 제어 CAN 송신부
...
┌──────────────────────────────────────────────────────────────┐
│              INPUT_ECU (센서 입력 수집)                      │
│       Infineon TC275 + Hitex Shield Buddy                    │
│       • ADC 센싱 • 버튼 입력 • CAN 송신                     │
└────────────────────────┬─────────────────────────────────────┘
                          │ CAN Bus
                          ▼
         ...
         ▼                                           ▼
    ┌────────────────────────┐        ┌─────────────────────────────┐
    │ ACT_ECU (출력 제어)    │        │ HMI_WEB Node (라즈베리파이) │
    │ TC275+Hitex Shield Buddy         │ • 미들웨어 (can_receiver.py)│
    │ • LED 제어              │        │ • 백엔드 (Node.js)          │
    │ • 버저                  │        │ • 프론트엔드 (React)        │
    │ • 서보 모터             │        │ • WebSocket 통신            │
    │ • MP3 재생              │        └─────────────────────────────┘
